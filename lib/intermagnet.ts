...
function longitude180(value: number) {
...
function parseHapiRows(payload: HapiPayload, code: string, datasetId: string): MagneticStationSeries {
  ...
  if (xIndex < 0 || yIndex < 0 || zIndex < 0) throw new Error(`${code}: INTERMAGNET HAPI no devolvió componentes XYZ.`);
...
export async function fetchIntermagnetSeries(code: string, start: Date, end: Date, signal?: AbortSignal) {
  ...
  const raw = await response.text();
  if (!response.ok) throw new Error(`${upper}: INTERMAGNET HTTP ${response.status}: ${raw.replace(/\s+/g, " ").slice(0, 140)}`);
  let payload: HapiPayload;
  try { payload = JSON.parse(raw) as HapiPayload; } catch { throw new Error(`${upper}: INTERMAGNET HAPI devolvió una respuesta no JSON.`); }
  const series = parseHapiRows(payload, upper, `INTERMAGNET:${datasetId}`);
  if (series.samples.length < 30) throw new Error(`${upper}: INTERMAGNET devolvió solo ${series.samples.length} minutos válidos.`);
  return series;
}
