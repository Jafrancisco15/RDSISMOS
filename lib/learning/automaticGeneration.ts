...
export async function runAutomaticProjectionGeneration(
  ...
  if (!getDb()) {
    summary.errors.push("DATABASE_URL no está configurada; no se puede emitir una proyección persistente.");
    return summary;
  }
    ...
    try {
      ...
      if (!storage.persisted) {
        summary.skipped.push({
          ...
          sourceMagnitude: event.magnitude,
      ...
      summary.predictionsCreated += capsule.destinations.filter((item) => Boolean(item.countryCode)).length;
      summary.created.push({
        sourceEventId: event.id,
        sourceMagnitude: event.magnitude,
        sourcePlace: event.place,
        sourceTime: event.timeUtc,
