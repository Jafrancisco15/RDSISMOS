...
import { TectonicStatePhase4 } from "./TectonicStatePhase4";
...
export function TectonicStatePhase3({ result, event }: { result: TectonicStatePhase3Result; event: EarthquakeEvent }) {
  ...
  const phase4Seed = useMemo(() => buildTectonicStatePhase4Seed(result), [result]);
  const ready = readinessStyle(result.readiness.label);

  return <>
    <section style={{ marginTop: 12, padding: 12, borderRadius: 15, border: "1px solid rgba(167,139,250,.25)", background: "linear-gradient(145deg,rgba(49,46,129,.18),rgba(2,8,23,.88))", minWidth: 0 }}>
    ...
    <TectonicStatePhase4 event={event} seed={phase4Seed} />
  </>;
}
