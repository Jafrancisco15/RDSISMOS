...
function normalizeUsgs(feature:UsgsFeature):EarthquakeEvent|null{
  ...
  return {
    id:feature.id, externalId:feature.id, sourceCatalog:"USGS ComCat", timeUtc:new Date(time).toISOString(),
    updatedUtc:new Date(num(p.updated,time)).toISOString(), latitude:num(c[1]), longitude:num(c[0]), depthKm:Math.max(0,num(c[2])),
    magnitude:num(p.mag), magnitudeType:text(p.magType,"M"), place, countryOrRegion:place.split(",").at(-1)?.trim()??place,
    eventType:text(p.type,"earthquake"), status:text(p.status,"reported"), network:text(p.net,"USGS"),
    locationSource:text(p.locationSource,"us"), magnitudeSource:text(p.magSource,"us"), sourceUrl:text(p.url,""),
  };
}
...
async function directUsgs(params:URLSearchParams,signal?:AbortSignal):Promise<EarthquakePage>{
  ...
  direct.set("format","geojson"); direct.set("eventtype","earthquake");
...
async function fetchPage(params:URLSearchParams,signal?:AbortSignal){
  try{
    ...
    return directUsgs(params,signal);
  }
}

function FloatingSection({events,anchor}:{events:EarthquakeEvent[];anchor:EarthquakeEvent}){
  const width=390,height=245,pad=34;
  const cos=Math.max(.2,Math.cos(anchor.latitude*Math.PI/180));
  const pts=events.map(e=>({e,x:(e.longitude-anchor.longitude)*111.32*cos,y:Math.max(0,e.depthKm)}));
  const maxX=Math.max(20,...pts.map(p=>Math.abs(p.x))); const maxDepth=Math.max(40,...pts.map(p=>p.y));
  const sx=(x:number)=>width/2+(x/maxX)*(width/2-pad); const sy=(d:number)=>pad+(d/maxDepth)*(height-pad*1.65);
  return <div className={styles.floatingSection}>
    <div className={styles.sectionTitle}><span>SECCIÓN HIPOCENTRAL DESPEGADA</span><b>E–O aproximado</b></div>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Sección vertical flotante de hipocentros">
      ...
      {[.25,.5,.75,1].map(f=><g key={f}><line x1={pad} y1={pad+(height-pad*1.65)*f} x2={width-pad} y2={pad+(height-pad*1.65)*f} className={styles.depthGrid}/><text x={5} y={pad+(height-pad*1.65)*f+4} className={styles.depthLabel}>{Math.round(maxDepth*f)} km</text></g>)}
      {pts.map(({e,x,y})=><circle key={e.id} cx={sx(x)} cy={sy(y)} r={Math.max(2.5,Math.min(7.5,e.magnitude*1.05))} fill={colorByDepth(e.depthKm)} opacity={.94}><title>{`M${e.magnitude.toFixed(1)} · ${e.depthKm.toFixed(1)} km · ${e.place}`}</title></circle>)}
      <text x={width/2} y={18} textAnchor="middle" className={styles.surfaceLabel}>superficie terrestre</text>
      <text x={width-7} y={height-6} textAnchor="end" className={styles.axisLabel}>E →</text><text x={7} y={height-6} className={styles.axisLabel}>← O</text>
    </svg>
    <small>La lámina está separada del planeta para hacer visible la geometría en profundidad. Los kilómetros son los del catálogo USGS.</small>
  </div>;
}
...
export function SequenceGlobeUnified(){
  ...
  const [size,setSize]=useState({width:920,height:650}); const [mode,setMode]=useState<Mode>("global");
  ...
  const [radiusKm,setRadiusKm]=useState(150); const [beforeDays,setBeforeDays]=useState(2); const [afterDays,setAfterDays]=useState(10);
  const [sequenceMinMagnitude,setSequenceMinMagnitude]=useState(1); const [depthExaggeration,setDepthExaggeration]=useState(2);
  ...
  useEffect(()=>{const n=containerRef.current;if(!n)return;const u=()=>setSize({width:Math.max(320,n.clientWidth),height:Math.max(560,Math.min(820,n.clientWidth*.82))});u();const o=new ResizeObserver(u);o.observe(n);return()=>o.disconnect();},[]);
  ...
  async function reconstruct(anchor:EarthquakeEvent){
    ...
    globeRef.current?.pointOfView({lat:anchor.latitude,lng:anchor.longitude,altitude:.75},850);
    try{const ms=Date.parse(anchor.timeUtc);const starttime=new Date(ms-beforeDays*DAY_MS).toISOString();const endtime=new Date(Math.min(Date.now(),ms+afterDays*DAY_MS)).toISOString();
      ...
      const gathered:EarthquakeEvent[]=[];for(let page=0;page<3;page++){const p=new URLSearchParams(base);p.set("offset",String(page*PAGE_SIZE+1));const r=await fetchPage(p);gathered.push(...r.events);if(!r.hasMore)break;}
      const unique=[...new Map(gathered.map(e=>[e.id,e])).values()].sort((a,b)=>Date.parse(a.timeUtc)-Date.parse(b.timeUtc)).slice(0,MAX_SEQUENCE_EVENTS);setSequenceEvents(unique);if(!unique.length)setError("USGS no devolvió eventos para ese radio y ventana temporal.");
  ...
  const times=useMemo(()=>sequenceEvents.map(e=>Date.parse(e.timeUtc)).filter(Number.isFinite),[sequenceEvents]); const t0=times.length?Math.min(...times):0,t1=times.length?Math.max(...times):0,cutoff=t0+(t1-t0)*(timelinePct/100);
  ...
  return <main className={styles.page}>
    <section className={styles.hero}><div><span className={styles.eyebrow}>RDSISMOS · SECUENCIA 3D</span><h1>{mode==="global"?"Selecciona un terremoto reciente":`Secuencia local · M${selected?.magnitude.toFixed(1)??"—"}`}</h1><p>{mode==="global"?"Toca un sismo sobre la Tierra. La secuencia se reconstruye sobre el mismo planeta y la sección hipocentral se despega visualmente del globo.":"El mapa conserva la ubicación real; la lámina flotante muestra profundidad hacia abajo para que la geometría no quede oculta dentro de la esfera."}</p></div><div className={styles.count}><span>{mode==="global"?"Eventos globales":"Hipocentros"}</span><strong>{active.length.toLocaleString("es-DO")}</strong><small>{mode==="global"?`${globalDays} días · M${globalMinMagnitude.toFixed(1)}+`:`${radiusKm} km · M${sequenceMinMagnitude.toFixed(1)}+`}</small></div></section>
    ...
    <section className={`${styles.globeCard} ${mode==="sequence"?styles.sequenceMode:""}`}><div className={styles.legend}>{colorMode==="depth"?<><span><i className={styles.shallow}/>&lt;35 km</span><span><i className={styles.mid}/>35–150 km</span><span><i className={styles.deep}/>&gt;150 km</span></>:<span>Azul = antiguo · rojo = reciente</span>}</div>
      <div className={styles.globe} ref={containerRef}>{(loading||sequenceLoading)&&<div className={styles.loading}>{sequenceLoading?"Consultando USGS y levantando la sección…":"Cargando sismos recientes…"}</div>}{!loading&&!sequenceLoading&&error&&<div className={styles.error}>{error}</div>}
        <Globe ref={globeRef} width={size.width} height={size.height} globeImageUrl={DAY_TEXTURE} backgroundColor="rgba(220,235,244,.06)" atmosphereColor="#8ed7f7" atmosphereAltitude={.13} pointsData={points} pointLat="lat" pointLng="lng" pointAltitude="altitude" pointRadius="radius" pointColor="color" pointLabel={(p:object)=>{const e=p as GlobePoint;return `<div class=\"globe-tooltip\"><strong>M${e.magnitude.toFixed(1)} · ${e.place}</strong><span>${formatUtc(e.timeUtc)} UTC</span><small>${e.depthKm.toFixed(1)} km de profundidad</small></div>`;}} onPointClick={(p:object)=>{const e=p as GlobePoint;setSelected(e);if(mode==="global")void reconstruct(e);}} ringsData={rings} ringLat="lat" ringLng="lng" ringAltitude={.006} ringColor={()=>["rgba(14,165,233,.95)","rgba(14,165,233,0)"]} ringMaxRadius="maxRadius" ringPropagationSpeed={0} ringRepeatPeriod={0} enablePointerInteraction />
        {mode==="sequence"&&selected&&visibleSequence.length>0&&<FloatingSection events={visibleSequence} anchor={selected}/>}</div>
      {mode==="global"?<div className={styles.hint}>{globalEvents.length?"Toca cualquier punto del globo para reconstruir su secuencia.":"Reintentando catálogo global…"}</div>:<aside className={styles.sequencePanel}><div className={styles.sequenceHead}><div><span>Secuencia seleccionada</span><h2>{selected?.place??"Evento"}</h2></div><strong>{visibleSequence.length}/{sequenceEvents.length}</strong></div><div className={styles.timelineRow}><button onClick={()=>{if(timelinePct>=100)setTimelinePct(0);setPlaying(v=>!v);}}>{playing?"Pausar":"▶ Reproducir"}</button><input type="range" min="0" max="100" value={timelinePct} onChange={e=>{setPlaying(false);setTimelinePct(Number(e.target.value));}}/><small>{times.length?formatUtc(new Date(cutoff).toISOString()):"—"}</small></div><div className={styles.sequenceStats}><span><b>{Math.max(0,...visibleSequence.map(e=>e.magnitude)).toFixed(1)}</b>Magnitud máx.</span><span><b>{Math.max(0,...visibleSequence.map(e=>e.depthKm)).toFixed(0)} km</b>Profundidad máx.</span><span><b>{radiusKm} km</b>Radio</span></div></aside>}
    </section>
  </main>;
}
