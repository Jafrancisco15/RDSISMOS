...
export function FreundExperimentalPanel({
  ...
  return <section style={panel}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "start" }}>
      <div>
        <div style={{ color: "#c084fc", fontSize: 10, fontWeight: 900, letterSpacing: ".1em" }}>FRIEDEMANN T. FREUND · POSITIVE-HOLE HYPOTHESIS</div>
        <h2 style={{ color: "white", margin: "5px 0 3px", fontSize: 20 }}>Prueba Freund · compatibilidad magnética experimental</h2>
        <p style={{ color: "#cbd5e1", fontSize: 11, lineHeight: 1.55, margin: 0, maxWidth: 940 }}>
      ...
      <span style={{ color: "#fda4af", border: "1px solid rgba(251,113,133,.35)", background: "rgba(127,29,29,.18)", borderRadius: 999, padding: "6px 9px", fontSize: 9, fontWeight: 900 }}>
        HIPÓTESIS NO VALIDADA PARA PREDICCIÓN
      </span>
    </div>

    <div style={{ marginTop: 12, padding: 10, borderRadius: 12, background: "rgba(15,23,42,.72)", color: "#ddd6fe", fontSize: 11, lineHeight: 1.55 }}>
    ...
    {!assessment ? <div style={{ marginTop: 12, color: "#94a3b8", fontSize: 11 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(180px,230px) 1fr", gap: 12, marginTop: 12, alignItems: "stretch" }}>
        <article style={{ borderRadius: 13, padding: 12, background: "rgba(30,27,75,.42)", border: "1px solid rgba(129,140,248,.2)" }}>
          <div style={{ color: "#a5b4fc", fontSize: 9, fontWeight: 900 }}>FREUND MAGNETIC COMPATIBILITY</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8 }}>
          {assessment.criteria.map((criterion) => <article key={criterion.id} title={criterion.note} style={{ borderRadius: 11, padding: 9, background: "rgba(2,8,18,.68)", border: "1px solid rgba(148,163,184,.12)" }}>
      ...
      <div style={{ marginTop: 10, color: "#94a3b8", fontSize: 9, lineHeight: 1.55 }}>
