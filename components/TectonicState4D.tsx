...
export function TectonicState4D() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/mantle-tomography?${tomographyParams}`, { cache: "force-cache", signal: controller.signal }).then((response) => readJsonResponse<MantleTomographyResponse & { error?: string }>(response).then((body) => { if (!response.ok) throw new Error(body.error ?? `Tomografía HTTP ${response.status}`); return body; })),
  ...
  return <section style={panel}>
    ...
    <div style={{ marginTop: 10, padding: 9, borderRadius: 11, border: "1px solid rgba(251,191,36,.22)", background: "rgba(120,53,15,.12)", color: "#fde68a", fontSize: 9.5, lineHeight: 1.5 }}>
      <b>Estado v0.1:</b> el cambio t₀→t₁ usa conteo sísmico y momento escalar liberado. No equivale a tensión acumulada. La inversión real de Vp/Vs/Q y deformación requiere waveforms, GNSS e InSAR; esas fases se mantendrán separadas hasta poder validarlas.
    </div>
    ...
    {summary && <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8, marginTop: 12 }}>
        ...
        <article style={card}><div style={{ color: "#fdba74", fontSize: 8.5, fontWeight: 900 }}>TOMOGRAFÍA</div><strong style={{ color: "white", fontSize: 21 }}>{summary.tomographyCells}</strong><div style={{ color: "#64748b", fontSize: 8.5 }}>SEISGLOB2 dVs · {core?.tomography.depthKm ?? tomographyDepth} km</div></article>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,290px),1fr))", gap: 10, marginTop: 12 }}>
        ...
        <article style={card}>
          ...
          <div style={{ marginTop: 7, color: "#cbd5e1", fontSize: 9.5, lineHeight: 1.6 }}>
            ...
            <b>SEISGLOB2:</b> anomalías dVs como estructura independiente del manto.<br />
        ...
        <article style={card}>
          <div style={{ color: "#34d399", fontSize: 9, fontWeight: 900 }}>SIGUIENTE NIVEL · INVERSIÓN</div>
          <div style={{ marginTop: 7, color: "#cbd5e1", fontSize: 9.5, lineHeight: 1.6 }}>
            <b>Fase 2:</b> recuperar waveforms Z/N/E EarthScope/FDSN y construir cobertura de rayos por voxel.<br />
      ...
      <details style={{ marginTop: 11, ...card, color: "#cbd5e1", fontSize: 9.5 }}>
        <summary style={{ cursor: "pointer", fontWeight: 900, color: "#e2e8f0" }}>Método y límites de Tectonic State 4D v0.1</summary>
        <div style={{ marginTop: 8, lineHeight: 1.6 }}>
          La ventana se divide en dos mitades iguales. Cada celda de 8° compara conteo y momento sísmico liberado entre t₀ y t₁. El corte tomográfico aporta contexto estructural pero no se usa para fabricar cambio temporal. Los mecanismos aumentan el soporte observacional. El score de cobertura mide disponibilidad de datos independientes; <b>no estima probabilidad de terremoto</b>. Esta versión no reemplaza Scope, ETAS, Historial ni los módulos de migración existentes.
        </div>
      </details>
    </>}
  </section>;
